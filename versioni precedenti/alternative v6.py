...
def process_image_to_temp(img_path, target_w, target_h):
    try:
        # pyvips - SUPER VELOCE con streaming
        ...
        if scale < 1.0:
            # thumbnail_image è ottimizzato per il downscaling
            img = img.thumbnail_image(target_w, height=target_h, size='down')

        tmp = tempfile.NamedTemporaryFile(delete=False, suffix=".png")
        tmp.close()

        # pyvips scrive direttamente su disco senza caricare tutto in RAM
...
class CardPrinterApp:
    ...
    def create_ui(self):
        ...
        # Header
        header = tk.Frame(self.root, bg='#c0392b', height=80)  # Rosso per TURBO mode
        header.pack(fill='x')
