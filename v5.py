...
def compute_grid_positions():
    positions = []
    # calcola quante carte stanno nella pagina
    cols = int((PAGE_W + GAP_MM) // (CARD_WIDTH_MM + GAP_MM))
    rows = int((PAGE_H + GAP_MM) // (CARD_HEIGHT_MM + GAP_MM))
    cols = max(1, cols)
    rows = max(1, rows)

    # larghezza e altezza totale della griglia
    grid_w = cols * CARD_WIDTH_MM + (cols - 1) * GAP_MM
    grid_h = rows * CARD_HEIGHT_MM + (rows - 1) * GAP_MM

    # origine centrata sia orizzontalmente che verticalmente
    x_start = (PAGE_W - grid_w) / 2
    y_start = (PAGE_H - grid_h) / 2

    for r in range(rows):
        y = y_start + r * (CARD_HEIGHT_MM + GAP_MM)
        for c in range(cols):
            x = x_start + c * (CARD_WIDTH_MM + GAP_MM)
            positions.append((x, y))
    return positions

def make_duplex_pdf_with_logo(image_folder, output_pdf, logo_path, progress_var):
    ...
    temp_files = temp_files[:len(images)]  # sicurezza
...
make_duplex_pdf_with_logo(image_folder, output_pdf, logo_path, progress_var)
